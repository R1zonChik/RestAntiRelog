...
import ru.leymooo.antirelog.util.PvPScoreboard;
...
public class PvPListener implements Listener {
    ...
    public void onBlockPlace(BlockPlaceEvent event) {
        ...
        if (pvpManager.isInPvP(player) &&
                (blockType == Material.DIAMOND_BLOCK ||
                        ...
                        blockType == Material.LODESTONE ||
                        blockType == Material.NETHERITE_BLOCK)) {

            event.setCancelled(true);
            player.sendMessage(ChatColor.RED + "Вы не можете ставить блоки привата во время PvP!");
            player.playSound(player.getLocation(), org.bukkit.Sound.BLOCK_NOTE_BLOCK_BASS, 1f, 1f);

            // Дополнительная проверка и удаление блока
    ...
    public void onDamageByEntity(EntityDamageByEntityEvent event) {
        ...
        // Обновляем Scoreboard для обоих игроков
        if (damager != null) {
            PvPScoreboard.updateScoreboard(damager, pvpManager.getOpponents(damager), pvpManager.getTimeRemainingInPvP(damager));
        }
        PvPScoreboard.updateScoreboard(target, pvpManager.getOpponents(target), pvpManager.getTimeRemainingInPvP(target));
    }
    ...
    public void onPlayerRespawn(PlayerRespawnEvent event) {
        ...
        Bukkit.getScheduler().runTaskLater(plugin, () -> {
            if (player.isOnline() && !pvpManager.isPlayerInPvP(player)) {
